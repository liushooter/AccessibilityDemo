...
import com.example.zhouzhihui.wechathelper.access.withdraw.Withdraw
...
fun dispatchEvent(event: AccessibilityEvent?, rootInActiveWindow: AccessibilityNodeInfo?) {
    ...
    when (eventType) {
        AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED -> com.example.zhouzhihui.wechathelper.access.packet.handleNotification(event)//64     1-->click
        AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED, AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED -> {//32 2048
            ...
            if (className == "com.tencent.mm.ui.LauncherUI" || className == "com.tencent.mm.ui.mogic.WxViewPager" || className == "android.widget.EditText"/* || className == "android.widget.ListView"*/) {
                com.example.zhouzhihui.wechathelper.access.packet.searchPacket(rootInActiveWindow)
                ...
                com.example.zhouzhihui.wechathelper.access.packet.openPacket(rootInActiveWindow)
            } else if (className == "com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyDetailUI") {
                com.example.zhouzhihui.wechathelper.access.packet.closePacket(rootInActiveWindow)
            }
        }
    }
